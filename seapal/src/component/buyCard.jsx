...
export default function BuyCard({ artwork, onClose }) {
  ...
  // Fallbacks to handle data safely if artwork prop properties are empty
  ...
  return (
    <div className="whatsapp-modal-overlay" onClick={onClose}>
      <div className="whatsapp-checkout-card" onClick={(e) => e.stopPropagation()}>
        ...
        >
          {/* Custom pure embedded inline SVG WhatsApp chat logo token */}
        ...
        <div className="whatsapp-qr-accordion">
          <button 
            className={`qr-toggle-header ${showQR ? "expanded" : ""}`}
            onClick={() => setShowQR(!showQR)}
          >
            <div className="qr-header-title-wrapper">
              <span className="qr-quad-icon">⛃</span> 
              QR code
            </div>
