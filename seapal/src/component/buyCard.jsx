...
export default function BuyCard({ artwork, onClose }) {
  ...
  return (
    <div className="whatsapp-modal-overlay" onClick={onClose}>
      <div className="whatsapp-checkout-card" onClick={(e) => e.stopPropagation()}>
        
        {/* Close Button */}
        <button className="close-checkout-btn" onClick={onClose}>×</button>

        {/* --- ARTWORK DATA ROW MATRIX (Image + Details Together) --- */}
        ...
        <div className="whatsapp-cta-instructions">
          ...
          <p className="cta-response-time">
            <img src={Clock} alt="Clock icon" className="clock-icon-svg" style={{width: '14px'}} /> 
            Usually replies within a few hours
          </p>
        </div>

        {/* Green Branded WhatsApp Redirect Link */}
        <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="whatsapp-redirect-btn">
          <svg className="whatsapp-inline-svg" viewBox="0 0 448 512">
        ...
        {/* --- ACCORDION DETAILED QR GENERATOR BAR --- */}
        <div className="whatsapp-qr-accordion">
          <button className={`qr-toggle-header ${showQR ? "expanded" : ""}`} onClick={() => setShowQR(!showQR)}>
            <div className="qr-header-title-wrapper">
              <span className="qr-quad-icon"><img src={QRCodeIcon} alt="QR Code icon" className="qr-code-icon-svg" style={{width: '14px'}} /></span> 
              QR code
            </div>
